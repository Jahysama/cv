...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  output.innerHTML =
    '<div class="terminal-welcome">Welcome to the portfolio terminal. Type "help" for available commands.</div>';

  // Mobile-specific: ensure keyboard appears on tap
  if (isMobileDevice()) {
    ...
    // Prevent zoom on double-tap for terminal area
    let lastTouchEnd = 0;
  ...
  function processCommand(command) {
    ...
    switch (mainCommand) {
      case "find":
        if (parts.length < 2) {
          output.innerHTML +=
            '<div class="terminal-error">Usage: find [search term] {optional: -page page_name}</div>';
        } else {
          let searchTerm = "";
          let page = "all";
  ...
  function findInContent(searchTerm, page = "all") {
    ...
    fetch(
      `/search?term=${encodeURIComponent(searchTerm)}&page=${encodeURIComponent(page)}`,
    )
      .then((response) => response.json())
      .then((data) => {
        console.log("Search results:", data);
  ...
  document.body.addEventListener("htmx:afterSwap", function (evt) {
    ...
    if (pendingHighlight) {
      ...
      highlightSearchTerms(
        pendingHighlight.searchTerm,
        pendingHighlight.results,
      );
